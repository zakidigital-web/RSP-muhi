...
import { Search, Check, Zap, AlertCircle, Layers, Printer } from 'lucide-react';
import { Student, PaymentType, Payment } from '@/lib/types';
import { toast } from 'sonner';
import { ReceiptPrint } from '@/components/pembayaran/ReceiptPrint';
...
export function QuickPaymentDialog({ open, onOpenChange }: QuickPaymentDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (items.length === 0) {
        toast.error('Tidak ada item pembayaran');
        return;
      }
      ...
      toast.success(
        created.length > 1
          ? `✅ ${created.length} pembayaran berhasil dicatat!`
          : formData.isPaidOff
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button
                  type="submit"
                  className="flex-1"
                  disabled={isSubmitting || selectedPaymentTypes.length === 0}
                >
                  {isSubmitting ? (
                    <>
                    ...
                    <>
                      ...
                      Simpan {selectedPaymentTypes.length > 0 && `(${formatCurrency(totalAmount)})`}
                    </>
                  )}
                </Button>
              </div>
